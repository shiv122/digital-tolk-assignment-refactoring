...
class BookingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //use config instead of env so it can use cached data and dont read .env file on every request 
        elseif ($request->__authenticatedUser->user_type == env('ADMIN_ROLE_ID') || $request->__authenticatedUser->user_type == env('SUPERADMIN_ROLE_ID')) {
            $response = $this->repository->getAll($request);
        }
        // what if both condition is false ? you'll get $response not defined
    ...
    public function distanceFeed(Request $request)
    {
        ...
        $flagged = $data['flagged'] === 'true' ? (empty($data['admincomment']) ? "Please, add comment" : 'yes') : 'no';
